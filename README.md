...
> [!NOTE]
> LLM-generated, can be done with scripts, for a personal use case, neither generic nor polished
...
- `make` builds the project with `cargo build --release`
- `make install` installs `compdir` into `~/bin/` and the zsh completion script `_compdir` into `~/.zfunc/`
